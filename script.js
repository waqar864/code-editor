...
function showPane(name){
    TAB_ORDER.forEach(k => {if(wraps[k]){
...
function buildwebSrcdoc(withTests=false){
    ...
    return `<!DOCTYPE html><html><head><style>${css}</style></head><body>${html}</body><script>${js}</script></html>`;
}

function runWeb(withTests=false){
  preview.srcdoc = buildwebSrcdoc(withTests);
  log(withTests ? 'Running with tests' : 'web preview updated');
}
